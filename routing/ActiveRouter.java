...
public abstract class ActiveRouter extends MessageRouter {
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		// @julianofischer
		// #essa é a mensagem que é recebida!
		
		/* Least Recently Forwarded */
		double timeForwarded = SimClock.getTime();
		m.setTimeForwarded(timeForwarded);
		
		//System.out.println("2 - tempo: "+timeForwarded);
		
		/* Most Forwarded - Global */
		int globalReplicas = m.getGlobalNumberOfReplicas();
		globalReplicas++;
	...
	protected boolean makeRoomForMessage(int size) {
		...
		while (freeBuffer < size) {
			...
			switch (this.getDropQueueMode()) {
			case DROP_MODE_FIFO:
				m = getOldestMessage(excludeMsgBeingSent); // don't remove msgs
				// being sent
				break;
			case DROP_MODE_RANDOM:
				m = getRandomMessage(excludeMsgBeingSent);
				break;
			case DROP_MODE_LRF:
	...
	/**
	 * Returns a random message in the message buffer (that is not being sent if
	 * excludeMsgBeingSent is true).
	 * 
	 * @param excludeMsgBeingSent
	 *            If true, excludes message(s) that are being sent.
	 * @return The randomized message or null if no message could be returned
	 *         (no messages in buffer or all messages in buffer are being sent
	 *         and exludeMsgBeingSent is true)
	 */
	protected Message getRandomMessage(boolean excludeMsgBeingSent) {
		Collection<Message> messages = this.getMessageCollection();
		List<Message> messagesList = new ArrayList<Message>(messages);
		Collections.shuffle(messagesList, new Random(SimClock.getIntTime()));
		Message randomized = null;
		// int i = 0;
		while (messagesList.size() > 0 && randomized == null) {
	...
	protected Message getLRFMessage(boolean excludeMsgBeingSent) {
		Collection<Message> messages = this.getMessageCollection();
	...
	protected void transferDone(Connection con) {
		/*
		 * Aqui podemos fazer atualizações no roteador *emissor* da mensagem
		 * após a transferência da mensagem ser concluída.
		 */

		/* @julianofischer - usado no LRF */
		//System.out.println("Transfer done...");
		
		double d = SimClock.getTime();
		String msgId = con.getMessage().getId();
		Message m = this.getMessage(msgId);
		m.setTimeForwarded(d);
		/* @julianofischer - MOFOL e MOFOG */
		int localReplicas = m.getLocalNumberOfReplicas();
		int globalReplicas = m.getGlobalNumberOfReplicas();
		localReplicas++;
		globalReplicas++;
		m.setLocalNumberOfReplicas(localReplicas);
		m.setGlobalNumberOfReplicas(globalReplicas);
		
		//System.out.println("FROM: "+m.getFrom().getAddress()+" TO:"+m.getTo().getAddress());
		//System.out.println("THIS: "+this.getHost().getAddress());
		//System.out.println("THIS(name): "+this.getHost());
		//System.out.println("1 - tempo ("+m.getId()+")"+":"+d);
		//System.out.println("2 - mofo: "+localReplicas);
		
		/*MDMF*/
		if(this instanceof ProphetRouter){
			//ProphetRouter pRouterFrom = (ProphetRouter)this;
			ProphetRouter pRouterTo = (ProphetRouter)((con.getOtherNode(getHost())).getRouter());
			double high = m.getHigherDeliveryProb();
			if(pRouterTo.getPredFor(m.getTo())>high){
				high = pRouterTo.getPredFor(m.getTo());
				m.setHigherDeliveryProb(high);
			}
		}
		
	}

	protected void setDropQueueMode(int dropQueueMode) {
		this.dropQueueMode = dropQueueMode;
	}

	protected int getDropQueueMode() {
		return dropQueueMode;
	}
